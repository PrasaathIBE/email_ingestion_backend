...
def build_processed_dataframe(payload: ProcessFileRequest) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if payload.options.split_multi_emails:
    ...
    if payload.options.deduplicate_by != "email":
...
def process_uploaded_file(payload: ProcessFileRequest) -> Dict[str, Any]:
...
def build_full_processed_payload(payload: ProcessFileRequest) -> Dict[str, Any]:
    processed_df, summary = build_processed_dataframe(payload)
